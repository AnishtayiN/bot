...
def db_init() -> None:
    ...
    with _db_lock:
        ...
        c.commit()  # executescript خودش INSERT/UPDATE را commit نمی‌کند، فقط CREATE را مدیریت می‌کند
...
def _ydl_base_opts() -> Dict[str, Any]:
    ...
    # --- PO Token / افزونه bgutil (اختیاری، از env تنظیم‌پذیر — yt-dlp 2026.08.19) ---
    # YTDLP_FETCH_POT   = never | auto | always   → «never» فراخوانی پلاگین PO-Token را خاموش می‌کند
    # YTDLP_DISABLE_POT_PLUGIN = 1/true           → معادل fetch_pot=never (بدون نیاز به حذف پلاگین)
    # YTDLP_PO_TOKEN    = "CLIENT.CONTEXT+TOKEN,..."  مثال: "mweb.gvs+XXXX,web.player=YYYY"
    #   (CONTEXT ∈ gvs|player|subs — برای عبور دستی از GVS 403 بدون پلاگین)
    yargs: Dict[str, Any] = {}
    fetch_pot = (os.getenv("YTDLP_FETCH_POT") or "auto").strip().lower()
    if fetch_pot in ("never", "auto", "always"):
        yargs["fetch_pot"] = [fetch_pot]
    if (os.getenv("YTDLP_DISABLE_POT_PLUGIN") or "").strip().lower() in ("1", "true", "yes"):
        yargs["fetch_pot"] = ["never"]
    pot = (os.getenv("YTDLP_PO_TOKEN") or "").strip()
    if pot:
        yargs["po_token"] = [t.strip() for t in pot.split(",") if t.strip()]
    if yargs:
        opts["extractor_args"] = {"youtube": yargs}
    return opts


# هر زنجیره باید با /best/worst تمام شود تا هیچ‌وقت «Requested format is not available» نگیریم.
...
# ⚠️ زنجیره‌ی صرفاً progressive («best[height<=X]/...») عملاً روی 720p سقف می‌شد،
#    چون یوتیوب بالای 720p فرمت تکی (v+a با هم) ندارد. از 2026.08.19 جفت DASH
#    (bv*+ba) + merge_output_format (که حالا «/»-جدا مجاز است: mp4/mkv/webm)
#    راه رسمی گرفتن 1080p است.
# ⚠️ دُمِ زنجیره باید هر دو حالت را پوشش دهد: کلاینت‌های DASH-only فرمت تکیِ
#    best/worst ندارند → بدون «/bv*+ba» آخر، خطای «Requested format is not available»
#    می‌گیرد؛ کلاینت‌های HLS-only هم با b/best/worst برمی‌گردند.
FORMAT_VIDEO = "bv*[height<=1080]+ba/b[height<=1080]/b/best/worst/bv*+ba"
QUALITY_FORMATS: Dict[str, str] = {
    "360": "bv*[height<=360]+ba/b[height<=360]/best/worst/bv*+ba",
    "720": "bv*[height<=720]+ba/b[height<=720]/best/worst/bv*+ba",
    "1080": "bv*[height<=1080]+ba/b[height<=1080]/best/worst/bv*+ba",
}

# --- نردبان تلاش یوتیوب (مبتنی بر yt-dlp 2026.08.19) ---
# کلاینت‌های معتبرِ این نسخه: web, web_safari, web_embedded, web_music, web_creator,
#   mweb, ios, visionos, android, android_vr, tv, tv_downgraded, tv_simply
#   (tv_embedded از 2026.01.29 حذف شده؛ «tv» بدون کوکی همه‌فرمت DRM می‌دهد → بی‌فایده)
# دیفالوت = visionos,web (بدون runtime جاوااسکریپت، web حذف می‌شود؛ android_vr از
#   2026.08.19 دیگر دیفالوت نیست #17461). با کوکی = web_embedded,tv_downgraded,web
# ⚠️ باگ باز #17389: tv_downgraded با کوکی → «The page needs to be reloaded»؛
#    دورزدن جامعه‌تأییدشده: player_client=default,-tv_downgraded یا web_safari,web_embedded (#17405)
# 📌 PO Token: mweb/tv_simply/web به GVS POT نیاز دارند → پلاگین bgutil (اختیاری، env بالا)
#    یا YTDLP_PO_TOKEN دستی؛ android_vr و web_safari (HLS) فعلاً بدون POT کار می‌کنند.
#    برای یوتیوب از 2025.11.12 حتماً runtime جاوااسکریپت (Deno≥2.3 / Node≥22؛ Bun deprecated)
#    + بسته‌ی yt-dlp-ejs لازم است.
_YT_FB_LADDER: List[Dict[str, Any]] = [
    {},                                              # 1) دیفالوت (visionos+web) + کوکی اصلی
    {"player_client": "default,-tv_downgraded"},     # 2) با کوکی، دورزدن باگ reload (#17389)
    {"player_client": "web_safari,web_embedded"},    # 3) ترکیب تأییدشده‌ی جامعه (#17405)
    {"player_client": "web_safari", "noc": True},    # 4) web_safari بدون کوکی (HLS بدون POT)
    {"player_client": "android_vr", "noc": True},    # 5) android_vr بدون کوکی (POT لازم ندارد)
    {"player_client": "mweb"},                       # 6) mweb + کوکی (با پلاگین POT عالی است)
    {"player_client": "mweb", "novi": True},         # 7) mweb + کوکی بدون VISITOR
    {"player_client": "tv_simply", "noc": True},     # 8) tv_simply (کوکی حساب پشتیبانی نمی‌کند)
    {"player_client": "visionos", "noc": True},      # 9) visionos بدون کوکی (JS-less)
    {"noc": True},                                   # 10) دیفالوت بدون کوکی
]
_YT_TRANSIENT_RE = re.compile(
    r"sign in|not a bot|needs to be reload|confirm you|login required|"
    r"requested format|unable to extract player|po ?token|missing_pot|"
    r"js runtime|javascript runtime|no supported .*runtime|"
    r"signature solving failed|n challenge solving failed|"
    r"rate-limited by youtube|precondition check failed|http error 403", re.I)
...
def _extract_with_fallback(url: str, base_opts: Dict[str, Any], *, download: bool = True) -> Any:
    ...
    had_cookies = "cookiefile" in base_opts and Path(base_opts["cookiefile"]).exists()
    novi_ready = COOKIES_NOVI_FILE != COOKIES_FILE and COOKIES_NOVI_FILE.exists()
    last_exc: Optional[Exception] = None
    for rung in _YT_FB_LADDER:
        opts = dict(base_opts)  # shallow copy — extractor_args باید جدا کپی شود
        ea = dict(opts.get("extractor_args") or {})
        pc = rung.get("player_client")
        if pc:
            yargs = dict(ea.get("youtube") or {})
            # پشتیبانی از لیست چندتایی و استثنا («default,-tv_downgraded»)
            yargs["player_client"] = [c.strip() for c in str(pc).split(",") if c.strip()]
            ea["youtube"] = yargs
        if ea:
...
# --- سلامت ابزارهای یوتیوب (نسخه، runtime جاوااسکریپت، بسته‌ی EJS) ---
try:
...
def js_runtime_status() -> Dict[str, Any]:
    """وضعیت پیش‌نیازهای یوتیوب را برمی‌گرداند: نسخه، runtime JS، بسته‌ی EJS.

    از yt-dlp 2025.11.12 حل چالش‌های جاوااسکریپت یوتیوب به یک runtime خارجی
    (Deno پیش‌فرض فعال است؛ Node/Bun/QuickJS با --js-runtimes) + اسکریپت‌های
    yt-dlp-ejs نیاز دارد. بدون آن، فرمت‌ها ناقص/خالی برمی‌گردند.
    """
    runtime_path = (shutil.which("deno") or shutil.which("node")
                    or shutil.which("nodejs") or shutil.which("bun"))
    ejs_ok = True
    ejs_err = ""
    try:
        ...
        ejs_ok = False
        ejs_err = str(exc)
    # پلاگین PO-Token (bgutil) اختیاری است؛ اگر نصب باشد رانگ‌های mweb/tv_simply فعال‌ترند
    pot_plugin = False
    try:
        import importlib.util
        pot_plugin = any(
            importlib.util.find_spec(m) for m in (
        ...
        pass
    ok = bool(runtime_path) and ejs_ok
    return {
        "ok": ok,
        "version": YTDLP_VERSION,
        "runtime": Path(runtime_path).name if runtime_path else "",
        "ejs": ejs_ok,
        "ejs_err": ejs_err,
...
def js_runtime_problem_text() -> str:
    """پیام هشدار (HTML) اگر پیش‌نیازهای یوتیوب کامل نبود؛ وگرنه رشته‌ی خالی."""
    st = js_runtime_status()
    if st["ok"]:
        return ""
    parts = []
    if not st["runtime"]:
        parts.append("runtime جاوااسکریپت (Deno/Node) نصب نیست")
    if not st["ejs"]:
        parts.append("بسته‌ی yt-dlp-ejs نصب نیست")
    return ("⚠️ <b>پیش‌نیازهای دانلود یوتیوب کامل نیست:</b> " + "، ".join(parts) +
            ".\n💡 روی سرور: <code>curl -fsSL https://deno.land/install.sh | sh</code> و "
            "<code>pip install -U \"yt-dlp>=2026.8.19\" yt-dlp-ejs</code>")
...
def friendly_dl_error(exc: BaseException) -> str:
    ...
    if "js runtime" in low or "javascript runtime" in low or ("no supported" in low and "runtime" in low) \
            or "signature solving failed" in low or "n challenge solving failed" in low:
        return ("⚙️ سرویس یوتیوب روی این سرور نیاز به نصب <b>Deno</b> و <b>yt-dlp-ejs</b> دارد. "
                "به ادمین اطلاع بده.")
    if "po token" in low or "pot required" in low or "missing a po" in low or "missing_pot" in low:
        return ("🔑 یوتیوب برای این ویدیو <b>PO-Token</b> می‌خواهد؛ افزونه‌ی bgutil روی سرور "
                "فعال نیست یا کوکی لازم است. کمی بعد دوباره امتحان کن.")
    if "needs to be reload" in low:
        return ("🍪 نشست کوکی یوتیوب منقضی/ناهمانگار شده (خطای «page needs to be reloaded»)؛ "
                "کوکی باید از پنجره‌ی ناشناس مرورگر و با همان IP سرور export شود. "
                "کمی بعد دوباره امتحان کن.")
    if "rate-limited by youtube" in low or "exceeding the rate limit" in low:
        return "⏳ یوتیوب درخواست‌های این سرور را موقتاً محدود کرده (حدود ۱ ساعت)؛ بعداً امتحان کن."
    if "sign in to confirm" in low or "not a bot" in low or ("age" in low and "restrict" in low):
        return "🔞 یوتیوب این درخواست را نیازمند ورود/تأیید دانست؛ کوکی معتبر حساب یوتیوب روی سرور لازم است."
    if "private" in low:
...
def probe_link_sync(url: str) -> Dict[str, Any]:
    ...
    opts = _ydl_base_opts()
    opts.pop("playlist_items", None)
    # format صریحِ همیشه‌موفق — چون فقط متادیتا می‌خواهیم؛ دُمِ زنجیره هر دو حالت
    # DASH-only (بدون فرمت تکی → bv*+ba) و HLS-only (فرمت تکی → best/worst) را پوشش می‌دهد.
    opts.update({"extract_flat": "in_playlist", "skip_download": True, "format": "best/worst/bv*+ba/b"})
...
def search_sync(query: str, platform: str, limit: int = 8) -> List[Dict[str, Any]]:
    ...
    opts.pop("playlist_items", None)
    opts.update({
        "extract_flat": "in_playlist",   # مقادیر مجاز API: False/True/'in_playlist'/'discard'/'discard_in_playlist'
        "skip_download": True,
        "format": "best/worst/bv*+ba/b",
...
async def deliver_media(
    ...
    if suffix in IMAGE_EXTS:
        ...
        try:
            ...
            if thumb_handle:
                thumb_handle.close()
                # اطمینان از بسته‌شدن کامل و حذف فایل موقت تامبنیل
                if thumb_p and thumb_p.exists():
                    try:
...
async def send_music_by_query(
    ...
    try:
        ...
        tag_mp3(
            ...
            album=clean_title(info.get("album") or ""),
            year=str(info.get("release_year") or info.get("upload_date") or "")[:4],
            genre=clean_title((info.get("genres") or [""])[0] if isinstance(info.get("genres"), list) and (info.get("genres") or [""]) else ""),
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    if data == "adm:stats":
        ...
        yt_health = "🟢 کامل" if _st["ok"] else "🔴 ناقص (Deno/EJS نصب شود)"
        txt = (
            f"📊 <b>آمار ربات</b>\n\n"
            ...
            f"📦 yt-dlp: <code>{_st['version']}</code> | JS: <code>{_st['runtime'] or '—'}</code> "
            f"| EJS: <code>{'✅' if _st['ejs'] else '❌'}</code> → {yt_health}"
        )
        try:
...
async def on_inline(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if cached and cached[0] > time.time():
            ...
            now = time.time()
            stale = [k for k, (exp, _) in INLINE_CACHE.items() if exp < now]
            for k in stale:
...
def main() -> None:
    ...
    _st = js_runtime_status()
    log.info("📦 yt-dlp %s | JS-runtime=%s | yt-dlp-ejs=%s",
             _st["version"], _st["runtime"] or "❌", "✅" if _st["ejs"] else "❌")
    if not _st["ok"]:
        log.warning("⚠️ دانلود یوتیوب ممکن است ناقص کار کند! %s",
