...
def find_downloaded_file(info: Dict[str, Any], prefer_ext: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        ...
        pass
    # آخرین تلاش: هر فایلی در پوشه دانلود با پسوند مناسب
    try:
        all_files = sorted(
            (f for f in DOWNLOAD_DIR.glob("*") if f.is_file()),
            key=lambda x: x.stat().st_mtime,
        ...
        if all_files and (time.time() - all_files[0].stat().st_mtime) < 900:
            return all_files[0]
    except Exception:
        pass
    return None
...
def dl_video_sync(url: str, prog: Dict[str, Any], platform: str = "other", download: bool = True,
    ...
    if info and info.get("_type") == "playlist":
        ...
        info = entries[0]
    # برای دانلود صوتی، اول .mp3 را بگرد
    prefer = ".mp3" if (platform == "soundcloud" or quality == "aud") else None
    path = find_downloaded_file(info, prefer_ext=prefer)
    if not path:
...
def dl_audio_sync(query: str, download: bool = True, search: str = "ytsearch1") -> Tuple[Dict[str, Any], Optional[Path]]:
    ...
    if info and info.get("_type") == "playlist":
        ...
        info = entries[0]
    # اول .mp3 را بگرد، بعد هر فایل جدیدی را قبول کن
...
async def cooldown_check_or_notice(uid: int, reply_coro_factory) -> bool:
    """True اگر مجاز باشد (و نوبت ثبت شود)."""
    left = cooldown_left(uid)
    if left > 0:
        try:
            await reply_coro_factory(L(uid, "cooldown", sec=left))
        except Exception:
            pass
        return False
    return True
...
async def handle_forwarded(update: Update, context: ContextTypes.DEFAULT_TYPE, msg: Message, user) -> None:
    ...
    if not has_media:
        return
    if not await cooldown_check_or_notice(user.id, lambda txt: msg.reply_text(txt)):
        return
    try:
...
async def submit_url(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user, url: str) -> None:
    ...
    if not await cooldown_check_or_notice(user.id, lambda txt: _send_html(context, chat_id, txt)):
        return
...
async def start_download(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user, url: str,
    ...
    if blocked:
        await _send_html(context, chat_id, blocked)
        return
    # ساند‌کلود همیشه صدا — کیفیت ویدیو بی‌معنی است
    if platform == "soundcloud" and quality not in ("aud", "best"):
        quality = "aud"
    emoji = PLATFORM_EMOJI.get(platform, "🌐")
...
async def on_content(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending and user.id in ADMIN_IDS and pending.get("act") in ("addc", "ban", "unban", "bcast1", "bcast2", "cd"):
        ...
        return
    if pending and (user.id not in ADMIN_IDS or pending.get("act") not in ("addc", "ban", "unban", "bcast1", "bcast2", "cd", "search")):
        ud.pop("pending", None)
        pending = None
...
async def deliver_media(
    ...
    # انتخاب اکشن مناسب بر اساس نوع فایل
    try:
        if suffix in AUDIO_EXTS or quality == "aud":
            await context.bot.send_chat_action(cid, ChatAction.UPLOAD_AUDIO)
        else:
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if matched:
        ...
        if matched in ("scs:", "yt:"):
            ...
            # برای نتایج جستجو، URL مستقیم را به عنوان کوئری جستجو می‌دهیم
            # چون send_music_by_query با ytsearch1/scsearch1 کار می‌کند
            query = title if title and title != "?" else url
            await send_music_by_query(context, chat_id, query, user.id,
